...
def train_trunknet(inputs):
    ...
    fac  = 1e-10*np.ones_like(dmin)#用于保正

    dmin[0,0,0] = np.min(u_train[:,:,0])#原始变量的上下界
    ...
    def hyper_initial_frequencies(layers):#rowdy激活函数中的可训练参数初始化
    ...
    for i in range(nt):
        ...
        A = matrix_init(3*G_dim,u_train.shape[0],keysm[i])#可训练矩阵
        W_trunk.append(w1)
        b_trunk.append(b1)
        Am.append(A)
    ...
    @jit
    def update(params, data, u, opt_state, tol):#参数更新和获取损失函数的值
        """ Compute the gradient for a batch and update the parameters """
        value, grads = value_and_grad(loss,argnums=0,has_aux=False)(params, data, u, tol)
        opt_state = opt_update(0, grads, opt_state)
        return get_params(opt_state), opt_state, value

    opt_init, opt_update, get_params = optimizers.adam(lr)#优化器

    opt_state = []
    for i in range(nt):#每个模型单独opt_state
