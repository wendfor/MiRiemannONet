...
def train_branchnet(inputs):
    ...
    def choose_trunk_model(folder,data,u, tol):
        ...
        for i in range(nt):
            ...
            for file in files:
                if 'model'+str(i+1)+'.' in file:
                    temp.append(file)#temp存某一个模型的所有训练
            f.append(temp)#f存所有模型的temp
        ...
        for i in range(nt):
            ...
            for j in range(len(f[i])):
                ...
                l2norm.append(l2)
            l2norm = jnp.array(l2norm)#某一个模型的所有训练的l2范数
            minimum = jnp.argmin(l2norm)#找l2范数最小的那个训练数据
    ...
    for i in range(nt):
        ...
        paramst.append(param)#所有模型的最优参数
    ...
    for i in range(nt):
        ...
        Rm.append(R)
        temp = jnp.einsum('ij,kjm->kim',R, Am)#B来逼近R*Am
        print("phi=\t",phi.shape,i)
        print("Am=\t",Am.shape,i)
        print("Q=\t",Q.shape,i)
        print("R=\t",R.shape,i)
        print("u_train1=\t",temp.shape,i)
        u_train1.append(temp)#所有模型的R*Am
    ...
    with open(filename, 'wb') as file:#存储R*
        pickle.dump(Rm, file)
